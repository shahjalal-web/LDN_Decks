...
import {
  ...
  Clock, Eye, TrendingUp, Activity, BarChart3,
} from 'lucide-react';
...
export default function DashboardPage() {
    ...
    endpoints.forEach((ep) => {
      fetch(`${API_URL}/${ep}`)
        ...
        .then((data) =>
          setCounts((prev) => ({
            ...prev,
            [ep]: Array.isArray(data) ? data.length : 0,
          }))
        )
        .catch(() =>
          setCounts((prev) => ({ ...prev, [ep]: 0 }))
        );
    });

    // Fetch contacts (needs auth)
      ...
      try {
        ...
        if (Array.isArray(data)) {
          setCounts((prev) => ({
            ...
            newContacts: data.filter((c: ContactItem) => c.status === 'new').length,
          }));
          setRecentContacts(data.slice(0, 5));
        }
      } catch {
        setCounts((prev) => ({ ...prev, contacts: 0, newContacts: 0 }));
      }
    }
  ...
  const statCards = [
    { label: 'Services', count: counts.services, icon: Wrench, color: '#e65100', href: '/dashboard/services' },
    { label: 'Blog Posts', count: counts.blogs, icon: FileText, color: '#1565c0', href: '/dashboard/blog' },
    { label: 'Showcases', count: counts.showcases, icon: Camera, color: '#6a1b9a', href: '/dashboard/showcase' },
    { label: 'Cities', count: counts.cities, icon: MapPin, color: '#2e7d32', href: '/dashboard/cities' },
    { label: 'Total Contacts', count: counts.contacts, icon: MessageSquare, color: '#00838f', href: '/dashboard/contacts' },
    { label: 'New Inquiries', count: counts.newContacts, icon: Activity, color: '#c62828', href: '/dashboard/contacts' },
  ];
          ...
          >
            <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
              <div className="flex items-center gap-2">
                ...
                {counts.newContacts !== null && counts.newContacts > 0 && (
                  <span
